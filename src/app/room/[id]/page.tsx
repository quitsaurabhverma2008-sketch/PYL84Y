...
export default function RoomPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="chat-container" style={{ background: 'var(--color-background)' }}>
      ...
      {/* HEADER */}
      <div style={{ padding: '12px 16px', background: 'var(--color-overlay)', borderBottom: '1px solid var(--color-card-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', backdropFilter: 'blur(24px)', position: 'sticky', top: 0, zIndex: 20, borderTop: '3px solid transparent', borderImage: 'linear-gradient(90deg, var(--color-primary), var(--color-secondary), var(--color-accent)) 1' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button onClick={leaveRoom} style={{ background: 'var(--color-primary-15)', border: '1px solid var(--color-primary-25)', color: 'var(--color-primary)', cursor: 'pointer', width: '36px', height: '36px', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg>
          </button>
          <div>
            <h2 style={{ fontSize: '16px', fontWeight: '700', fontFamily: 'var(--font-heading)', background: 'linear-gradient(135deg, var(--color-primary), var(--color-secondary))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>PYL84Y</h2>
        ...
        <div style={{ display: 'flex', gap: '6px' }}>
          {!isInCall && (
            <>
              <button onClick={() => initiateCall('voice')} style={{ background: 'var(--color-accent-15)', border: '1px solid var(--color-accent-30)', color: 'var(--color-accent)', cursor: 'pointer', padding: '8px 12px', borderRadius: '10px', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '4px', transition: 'all 0.2s' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                Voice
              </button>
              <button onClick={() => initiateCall('video')} style={{ background: 'var(--color-primary-15)', border: '1px solid var(--color-primary-30)', color: 'var(--color-primary)', cursor: 'pointer', padding: '8px 12px', borderRadius: '10px', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '4px', transition: 'all 0.2s' }}>
          ...
          <button onClick={() => setShowMenu(!showMenu)} style={{ background: 'var(--color-primary-15)', border: '1px solid var(--color-primary-25)', color: 'var(--color-primary)', cursor: 'pointer', padding: '8px 10px', borderRadius: '10px', transition: 'all 0.2s' }}>
      ...
      <div className="messages-area" ref={messagesRef} style={chatBg ? { backgroundImage: `url(${chatBg})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundBlendMode: 'multiply', backgroundColor: 'rgba(15,23,42,0.85)' } : undefined}>
        {messages.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: '16px', color: 'var(--color-text-muted)' }}>
            <div style={{ width: '64px', height: '64px', borderRadius: '20px', background: 'linear-gradient(135deg, var(--color-primary-20), var(--color-accent-20))', border: '1px solid var(--color-primary-30)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--color-primary)" strokeWidth="1.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
            </div>
            <p style={{ fontSize: '15px', fontWeight: '600' }}>No messages yet. Say hello!</p>
      ...
      {!isInCall && (
        <div className="input-area">
          ...
          <button onClick={sendMessage} disabled={!inputText.trim()}
            style={{ background: inputText.trim() ? 'linear-gradient(135deg, var(--color-primary), var(--color-secondary), var(--color-accent))' : 'var(--color-primary-15)', border: inputText.trim() ? 'none' : '1px solid var(--color-primary-25)', color: inputText.trim() ? 'white' : 'var(--color-text-muted)', cursor: 'pointer', padding: '10px 14px', borderRadius: '12px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: inputText.trim() ? '0 4px 20px var(--color-glow-strong)' : 'none', backgroundSize: '200% 200%', animation: inputText.trim() ? 'gradientShift 3s ease infinite' : 'none', transition: 'all 0.25s' }}>
